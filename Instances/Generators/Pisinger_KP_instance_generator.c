...
void showitems(item *f, item *l, stype c, char name_f[50])
{
  item *i;
  stype ps, ws;
  FILE *out;
...
char file_name[50],ch[10];

for(i=0;i<10;i++)
{
  // concatenate KP and n
  strcpy(file_name,"KP_"); sprintf(ch,"%d",n); strcat(file_name,ch);

  // concatenate BGP_n and number of instance 
  strcat(file_name,"_"); sprintf(ch,"%d",i);strcat(file_name,ch);
	
  // Add extension "txt"
  strcat(file_name,".txt");
  printf("\n name file : %s",file_name);
  f = palloc(n, sizeof(item));
  l = f + n-1;
  c = maketest(f, l, r, type, i, S); 
  showitems(f, l, c, file_name);
  pfree(f);
}
}
